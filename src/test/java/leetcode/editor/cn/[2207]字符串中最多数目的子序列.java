package leetcode.editor.cn;

import org.junit.Test;

import static org.junit.Assert.assertEquals;

class SolutionTest2207 {
//给你一个下标从 0 开始的字符串 text 和另一个下标从 0 开始且长度为 2 的字符串 pattern ，两者都只包含小写英文字母。
//
// 你可以在 text 中任意位置插入 一个 字符，这个插入的字符必须是 pattern[0] 或者 pattern[1] 。注意，这个字符可以插入在
//text 开头或者结尾的位置。
//
// 请你返回插入一个字符后，text 中最多包含多少个等于 pattern 的 子序列 。
//
// 子序列 指的是将一个字符串删除若干个字符后（也可以不删除），剩余字符保持原本顺序得到的字符串。
//
//
//
// 示例 1：
//
//
//输入：text = "abdcdbc", pattern = "ac"
//输出：4
//解释：
//如果我们在 text[1] 和 text[2] 之间添加 pattern[0] = 'a' ，那么我们得到 "abadcdbc" 。那么 "ac" 作为子序
//列出现 4 次。
//其他得到 4 个 "ac" 子序列的方案还有 "aabdcdbc" 和 "abdacdbc" 。
//但是，"abdcadbc" ，"abdccdbc" 和 "abdcdbcc" 这些字符串虽然是可行的插入方案，但是只出现了 3 次 "ac" 子序列，所以不
//是最优解。
//可以证明插入一个字符后，无法得到超过 4 个 "ac" 子序列。
//
//
// 示例 2：
//
//
//输入：text = "aabb", pattern = "ab"
//输出：6
//解释：
//可以得到 6 个 "ab" 子序列的部分方案为 "aaabb" ，"aaabb" 和 "aabbb" 。
//
//
//
//
// 提示：
//
//
// 1 <= text.length <= 10⁵
// pattern.length == 2
// text 和 pattern 都只包含小写英文字母。
//
//
// Related Topics贪心 | 字符串 | 前缀和
//
// 👍 57, 👎 0bug 反馈 | 使用指南 | 更多配套插件
//
//
//
//

    public static
            //leetcode submit region begin(Prohibit modification and deletion)
    class Solution {
        public long maximumSubsequenceCount(String text, String pattern) {
            long first = 0;
            long second = 0;
            long ans = 0;
            for (char c : text.toCharArray()) {
                if (c == pattern.charAt(0)) {
                    first++;
                }
                if (c == pattern.charAt(1)) {
                    second++;
                    ans += first;
                }
            }
            return ans + (pattern.charAt(0) == pattern.charAt(1) ? 0 : Math.max(first, second));
        }
    }
//leetcode submit region end(Prohibit modification and deletion)

    //Do some Test
    public static class TestClass {

        private final Solution solution = new Solution();

        @Test
        public void defaultSolutionTest() {
            Solution solution = new Solution();
        }

        @Test
        public void testMaximumSubsequenceCountExample1() {
            String text = "abdcdbc";
            String pattern = "ac";
            assertEquals(4, solution.maximumSubsequenceCount(text, pattern));
        }

        @Test
        public void testMaximumSubsequenceCountExample2() {
            String text = "aabb";
            String pattern = "ab";
            assertEquals(6, solution.maximumSubsequenceCount(text, pattern));
        }

        @Test
        public void testMaximumSubsequenceCountExample3() {
            String text = "";
            String pattern = "rr";
            assertEquals(496, solution.maximumSubsequenceCount(text, pattern));
        }

    }
}
